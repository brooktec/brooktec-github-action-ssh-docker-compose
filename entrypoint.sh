...
remote_command="set -e ; log() { echo '>> [remote]' \$@ ; } ; log 'Creating workspace directory...' ; mkdir -p \"\$HOME/\$WORKSPACE\" ; trap EXIT ; log 'Unpacking workspace...' ; tar -C \"\$HOME/\$WORKSPACE\" -xjv ; log 'Launching docker compose...' ; cd \"\$HOME/\$WORKSPACE\" ; docker compose -f \"$DOCKER_COMPOSE_FILENAME\" pull ; docker compose -f \"$DOCKER_COMPOSE_FILENAME\" up -d --remove-orphans --build"

if $DOCKER_COMPOSE_DOWN ; then
  remote_command="set -e ; log() { echo '>> [remote]' \$@ ; } ; log 'Creating workspace directory...' ; mkdir -p \"\$HOME/\$WORKSPACE\" ; trap EXIT ; log 'Unpacking workspace...' ; tar -C \"\$HOME/\$WORKSPACE\" -xjv ; log 'Launching docker compose...' ; cd \"\$HOME/\$WORKSPACE\" ; docker compose -f \"$DOCKER_COMPOSE_FILENAME\" down"
fi
